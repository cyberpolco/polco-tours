...
afterAll(async () => {
  // Guard: if beforeAll failed before orgA/orgB/userAId/userBId were
  // assigned, Prisma silently drops the undefined where-clause value,
  // turning cleanup into an unscoped deleteMany that wipes the whole table
  // -- this has hit real production data twice. Skip cleanup entirely
  // rather than risk it.
  if (!orgA || !orgB || !userAId || !userBId) {
