...
export async function createGuestBookingAction(
  ...
  try {
    ...
    // onto User.preferredLocale (guest-locale.ts) -- every automated
    // booking/payment/visa email reads that field to pick its language
    // (notify()/notifyEmail()), and nothing else ever wrote it for a guest
    // checkout session, so it silently stayed at the schema default (EN)
    // regardless of what language the guest actually used.
