...
// DR-054 (revised same session): replaces /book/[departureId] as the guest
// entry point for a PREDEFINED_PACKAGE booking -- there's no pre-existing
// Departure to pick here at all, the guest's own chosen start date
// (collected in BookingForm) is what creates one
// (catalogService.createDepartureForBooking, via
// bookingService.createHoldWithDates). Trip length is staff-set
// (pkg.durationDays, set at package creation), never a guest choice --
// hence the durationDays != null gate below, mirroring the priceMinor gate.
// /book/[departureId] itself is left unchanged for any real, staff-
// pre-scheduled Departure a direct link might still point at -- this route
// is additive, not a replacement of that one.
