...
// with severity. At sm+ these sit on top of a translucent glass card over a
// photo (weather-glass.ts), so those opacities are much lower than a wash on
// a solid bone page would need -- anything heavier turns the glass opaque
// and defeats the effect. Below sm there's no photo (explicit user request:
// the hero photo is hidden on mobile, see weather-glass.ts), so the card is
// plain bone again and gets back its higher, pre-photo opacities. Plain
// string classes (not computed per-render, and both breakpoints' classes
// spelled out in full rather than templated) so Tailwind's static analysis
// picks them up -- this is one of the few places outside tailwind.config.ts
// itself that needs every class spelled out literally.
export function weatherCardTint(category: ConditionCategory): string {
  switch (category) {
    case 'clear':
      return 'bg-gradient-to-br from-forest-soft via-forest/10 to-transparent sm:from-forest-soft/30';
    case 'cloudy':
      return 'bg-gradient-to-br from-gray-200/70 via-gray-100/40 to-transparent sm:from-white/25 sm:via-white/10';
    case 'rain':
      return 'bg-gradient-to-br from-gray-400/30 via-gray-300/25 to-transparent sm:from-slate-300/25 sm:via-slate-200/10';
    case 'storm':
      return 'bg-gradient-to-br from-gray-600/30 via-gray-500/20 to-transparent sm:from-slate-700/35 sm:via-slate-600/15';
    case 'snow':
      return 'bg-gradient-to-br from-gray-100/80 via-gray-50/50 to-transparent sm:from-white/40 sm:via-white/15';
  }
}
