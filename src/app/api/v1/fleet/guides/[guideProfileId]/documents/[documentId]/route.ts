...
// Same anti-BOLA confirmation as the vehicle/driver documents routes: verify
// the document belongs to this guide profile before streaming it.
export const GET = withAuth<Params>('fleet.read', async (ctx, _req, { guideProfileId, documentId }) => {
  const documents = await fleetService.listGuideDocuments(ctx, guideProfileId);
