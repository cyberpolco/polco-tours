...
// "In progress" section on the old combined /staff/schedule page.
export default async function InProgressAssignmentsPage({ searchParams }: Props) {
  ...
  const inProgressRows = allRows.filter((r) => r.progress?.status === 'IN_PROGRESS');

  const filtered = inProgressRows.filter((r) => {
  ...
  function hrefWith(overrides: Record<string, string | undefined>): string {
    ...
    const s = qs.toString();
    return s ? `/staff/schedule/in-progress?${s}` : '/staff/schedule/in-progress';
  }

  return (
    <div className="max-w-4xl space-y-6">
      <BackLink href="/staff/schedule">back to my schedule</BackLink>
      <PageHeader eyebrow="My schedule" title="In Progress Assignments" />
      ...
      {rows.length === 0 ? (
        <p className="text-mist">No in-progress assignments match these filters.</p>
      ) : (
        <AssignmentsSection rows={rows} />
      )}
