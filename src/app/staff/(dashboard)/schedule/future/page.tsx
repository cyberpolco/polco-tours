...
// "Upcoming" section on the old combined /staff/schedule page.
export default async function FutureAssignmentsPage({ searchParams }: Props) {
  ...
  const futureRows = allRows.filter((r) => r.progress?.status === 'NOT_STARTED');

  const filtered = futureRows.filter((r) => {
  ...
  function hrefWith(overrides: Record<string, string | undefined>): string {
    ...
    const s = qs.toString();
    return s ? `/staff/schedule/future?${s}` : '/staff/schedule/future';
  }

  return (
    <div className="max-w-4xl space-y-6">
      <BackLink href="/staff/schedule">back to my schedule</BackLink>
      <PageHeader eyebrow="My schedule" title="Future Assignments" />

      <form method="get" action="/staff/schedule/future" className="grid grid-cols-2 gap-4 sm:grid-cols-3">
        ...
        <div className="col-span-2 flex items-end gap-3 sm:col-span-1">
          ...
          {(q || departureStatus) && (
            <Link href="/staff/schedule/future" className="text-sm text-mist hover:underline">
              Clear filters
            </Link>
          )}
        </div>
      ...
      {rows.length === 0 ? <p className="text-mist">No future assignments match these filters.</p> : <AssignmentsSection rows={rows} />}
