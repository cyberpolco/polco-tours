    ...
    <div className="max-w-md">
      ...
      <form action={createPackageAction} className="mt-6 space-y-4">
        ...
        {/* DR-114: staff upload real files (catalogService.uploadPackageImage,
            ...
            set. DR-172: up to 3 -- shown as a slideshow on the guest
            package card, in the order uploaded. DR-264: each file now
            uploads directly to Blob from the browser and is handed back
            already-compressed as a hidden `imageUrls` input, rather than
            proxying raw bytes through this Server Action. */}
        <FormField label={t('image')} htmlFor="images" optional>
          <PackageImageUploader namespace="StaffPackages" fieldName="imageUrls" />
        </FormField>
