...
// paying (DR-024) -- these don't expire and hold no seat, so unlike the
// regular bookings list they need active staff attention, not just a record.
// Same Table/Badge pattern as the bookings list; reuses bookingService.list
// (org-wide manifest for staff) rather than a new service method, filtered
// here since the org's booking volume is small (DR-005 single-tenant launch).
export default async function QuoteRequestsPage() {
  const ctx = await requireStaffContext('booking.read');
  const quotes = (await bookingService.list(ctx)).filter((b) => b.status === 'QUOTE_REQUESTED');
